              ...
              <div key={colIndex} className="flex flex-col gap-[10px] flex-1">
                {columnItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      {/* 이미지 컨테이너 */}
                      <div className={`relative w-full overflow-hidden bg-slate-900 transition-all duration-700 ${item.vertical ? 'aspect-[9/16]' : 'aspect-[16/10]'}`}>
                        <div className="absolute top-3 right-3 z-20 flex items-center gap-1.5">
                          {getVideoPlatform(item.videoUrl) === 'instagram' && (
                            <div className="bg-gradient-to-r from-purple-600 via-pink-600 to-rose-500 text-white text-[9px] font-bold px-2 py-1 rounded-full uppercase tracking-wider flex items-center gap-1 shadow-sm">
                          ...
                          {item.vertical && getVideoPlatform(item.videoUrl) !== 'instagram' && (
                            <div className="bg-black/60 backdrop-blur-sm text-white text-[9px] font-bold px-2 py-1 rounded-full uppercase tracking-wider flex items-center gap-1 shadow-sm">
                        ...
                        <img
                          ...
                          loading="lazy"
                          onError={(e) => {
                            // 이미지 로드 실패 시 그라디언트 기본 플레이스홀더로 대체
                            e.currentTarget.src = 'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?q=80&w=1470&auto=format&fit=crop';
                          }}
